...
| Ruolo | Accesso |
|-------|---------|
| **Amministratore** | Tutto: Dashboard, Utenti, Operatori, Impostazioni complete (SMTP, Notifiche, Backup) |
| **Segreteria** | Dashboard, Utenti, Impostazioni limitate (solo widget) |
...
- Riepilogo scadenze certificati:
  ...
  - Già scaduti (grigio)
  - Ogni card è cliccabile per filtrare utenti

**Attività recenti:** Log temporale operazioni (creazione, modifica, eliminazione utenti/certificati, login)

**Bacheca:** Note condivise tra operatori (max 500 caratteri, eliminabili solo dall'autore)

### 3. GESTIONE UTENTI (`/users`)
...
- Export singolo utente come PDF con QR code
...
**Backup e Ripristino:**
- Backup completo in ZIP (utenti, certificati allegati, operatori, settings, bacheca)
- Struttura: `gestcert_BK_[timestamp].zip` con cartelle per utente
...
- Notifica utenti (email ai lavoratori)
...
3. Contiene: JSON + cartelle utenti con certificati allegati

### Ripristino
1. Impostazioni → "Ripristina Backup"
2. Seleziona ZIP → Anteprima (n. utenti, certificati, data)
...
1. Essere divisa per sezioni (Dashboard, Utenti, Operatori, Impostazioni)
